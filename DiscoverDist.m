function [D, PD] = DiscoverDist(data,sortby,varargin)
%DISCOVERDIST Fit all valid parametric probability distributions to data.
%   [D PD] = DISCOVERDIST(DATA) fits all valid parametric probability
...
%
%   [...] = DISCOVERDIST(DATA,SORTBY) returns the struct of valid distributions
...
%
%   [...] = DISCOVERDIST(...,'DISCRETE') specifies it is a discrete
%   distribution and does not attempt to fit a continuous distribution
%   to the data
%
%   [...] = DISCOVERDIST(...,'PDF') or (...,'CDF') plots either the PDF or CDF
...
%
%   Note: DISCOVERDIST does not handle nonparametric kernel-smoothing,
...
%        [D PD] = DiscoverDist(data,'PDF');   %Compute and plot results
...
%        [D PD] = DiscoverDist(values,'NLogL','frequency',freq,'PDF','DISCRETE');
%        PD{1}
%
%  EXAMPLE 3
...
%        [D PD]= DiscoverDist(data,'PDF','DISCRETE');
%        PD{1}
%
%  EXAMPLE 4
...
%        [D1 PD1] = DiscoverDist(data,'n',10,'DISCRETE','PDF'); %Force binomial
%        [D2 PD2] = DiscoverDist(data,'DISCRETE','PDF');       %May be binomial
...
distname={'lognormal', 'weibull', 'extreme value', 'normal'};
